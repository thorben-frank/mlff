...
from mlff.src.training.lr_decay import get_lr_decay
...
class CustomTrainState(struct.PyTreeNode):
    ...
    def apply_gradients(self, *, grads, **kwargs):
        ...
        _cd_decay = self.lr_decay_fn(step=self.step)
...
def create_train_state(module: Module,
                       ...
                       scheduled_lr_decay: Dict = None) -> Tuple[CustomTrainState, Dict]:
    """
    ...
    lrd_fn = get_lr_decay(*tuple(scheduled_lr_decay.items())[0]) if scheduled_lr_decay is not None else def_lr_decay_fn
